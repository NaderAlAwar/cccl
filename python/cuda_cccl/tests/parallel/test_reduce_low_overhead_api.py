...
def test_device_reduce():
    ...
    reduce_into = parallel.algorithms.reduce_into_low_overhead(
        d_output, d_output, min_op, h_init
    )

    # Determine temporary device storage requirements
    temp_storage_size = reduce_into(
        None, 0, d_input.data.ptr, d_output.data.ptr, len(d_input), h_init
    )
...
#     temp_storage_size = reduce_into(
#         None, iterator, d_output, len(values), h_init)
#     d_temp_storage = cp.empty(temp_storage_size, dtype=np.uint8)
...
#     temp_storage_size = reduce_into(
#         None, constant_it, d_output, num_items, h_init)
#     d_temp_storage = cp.empty(temp_storage_size, dtype=np.uint8)
...
#     temp_storage_size = reduce_into(
#         None, first_it, d_output, num_items, h_init)
#     d_temp_storage = cp.empty(temp_storage_size, dtype=np.uint8)
...
#     temp_storage_size = reduce_into(
#         None, transform_it, d_output, num_items, h_init)
#     d_temp_storage = cp.empty(temp_storage_size, dtype=np.uint8)
