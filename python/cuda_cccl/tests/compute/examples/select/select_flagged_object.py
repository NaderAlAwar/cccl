...
from cuda.compute.algorithms import make_select_flagged
...
selector = make_select_flagged(d_in, d_flags, d_out, d_num_selected, is_selected)

# Get required temp storage
temp_storage_bytes = selector(
    None, d_in, d_out, d_num_selected, is_selected, len(d_in), d_flags
)
d_temp_storage = cp.empty(temp_storage_bytes, dtype=cp.uint8)

# Execute flagged select
selector(d_temp_storage, d_in, d_out, d_num_selected, is_selected, len(d_in), d_flags)
