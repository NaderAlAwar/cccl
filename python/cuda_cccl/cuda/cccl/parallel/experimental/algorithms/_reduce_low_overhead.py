...
class _ReduceLowOverhead:
    ...
    def __call__(
        ...
        self.d_in_cccl.state = d_in_ptr
        self.d_out_cccl.state = d_out_ptr

        try:
            h_init_data = h_init.data
        except AttributeError:  # This is for GpuStruct
            h_init_data = h_init._data.data

        self.h_init_cccl.state = h_init_data.cast("B")
...
def reduce_into_low_overhead(
    ...
    """
    return _ReduceLowOverhead(d_in, d_out, op, h_init)
