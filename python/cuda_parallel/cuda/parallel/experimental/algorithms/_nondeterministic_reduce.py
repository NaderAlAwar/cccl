...
class _NondeterministicReduce:
    ...
    def __init__(
        ...
        self.build_result = call_build(
            _bindings.DeviceNondeterministicReduceBuildResult,
            self.d_in_cccl,
            self.d_out_cccl,
            self.op_wrapper,
            self.h_init_cccl,
    ...
    def __call__(self, temp_storage, d_in, d_out, num_items, h_init, stream=None):
        ...
        # self.d_out_cccl.state = d_out.data.ptr
...
def nondeterministic_reduce_into(
    d_in: DeviceArrayLike | IteratorBase,
    d_out: DeviceArrayLike,
    op: Callable,
    h_init: np.ndarray,
    ...
    """
    return _NondeterministicReduce(d_in, d_out, op, h_init)
