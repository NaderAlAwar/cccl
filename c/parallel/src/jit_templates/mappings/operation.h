...
struct parameter_mapping<cccl_op_t>
{
  ...
  {
    return std::string("cccl_op_t_mapping{.is_stateless = ") + std::to_string(op.type == cccl_op_kind_t::CCCL_STATELESS)
         + std::string(", .size = ") + std::to_string(op.size) + std::string(", .alignment = ")
         + std::to_string(op.alignment) + std::string(", .operation = ") + std::string(op.name) + std::string("}");
  }
  ...
  static std::string aux(template_id<Traits>, cccl_op_t op)
  {
    return std::string(R"(
        extern "C" __device__ void )")
         + std::string(op.name) + std::string(R"();
        )");
  }
};
#endif
