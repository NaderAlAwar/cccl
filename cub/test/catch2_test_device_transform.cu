...
C2H_TEST("DeviceTransform::Transform uncommon types", "[device][device_transform]", uncommon_types)
{
  ...
  c2h::device_vector<int8_t> a(num_items); // put some bytes at the front, so SMEM has to handle
...
C2H_TEST("DeviceTransform::Transform add five streams", "[device][device_transform]")
{
  ...
  c2h::device_vector<std::int8_t> a(num_items);
  c2h::device_vector<std::int16_t> b(num_items);
  c2h::device_vector<std::int32_t> c(num_items);
  c2h::device_vector<std::int64_t> d(num_items);
  c2h::device_vector<float> e(num_items);
  ...
  c2h::host_vector<double> reference_h(num_items);
