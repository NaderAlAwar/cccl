...
void s5_scan_benchmark(nvbench::state& state, nvbench::type_list<T>)
{
  ...
  thrust::device_vector<T> d_A_in  = s5_operator_segmented_actual::generate_data<T>(total_elements);
  thrust::device_vector<T> d_Bu_in = s5_operator_segmented_actual::generate_data<T>(total_elements);

  state.add_element_count(timesteps * state_dim);

  size_t temp_storage_bytes = 0;
  auto [input_iter, begin_offsets, end_offsets] =
    s5_operator_segmented_actual::setup_scan<T, state_dim>(d_A_in, d_Bu_in, timesteps, temp_storage_bytes);
  ...
  state.exec(nvbench::exec_tag::sync, [&](nvbench::launch& launch) {
    ...
    s5_operator_segmented_actual::run_scan<T, state_dim>(
