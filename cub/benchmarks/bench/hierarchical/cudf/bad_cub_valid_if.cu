                                ...
                                const thrust::device_vector<int>& valid_count)
{
  ...
  {
    throw std::runtime_error("bad_cub_valid_if correctness check failed: unexpected number of mask words.");
  }
  ...
  for (std::size_t word = 0; word < h_mask_words.size(); ++word)
  {
    ...
    {
      throw std::runtime_error("bad_cub_valid_if correctness check failed: unexpected bitmask word.");
    }
  }

  if (h_valid_count.size() != 1 || h_valid_count[0] != expected_valid_count)
  {
    throw std::runtime_error("bad_cub_valid_if correctness check failed: unexpected valid count.");
  }
}
} // namespace

void bad_cub_valid_if(nvbench::state& state)
try
{
  ...
  auto counted_mask_words =
    cuda::make_transform_output_iterator(d_mask_words, [d_valid_count] __device__(std::uint32_t word) -> std::uint32_t {
      atomicAdd(d_valid_count, __popc(word));
      return word;
    });
...
NVBENCH_BENCH(bad_cub_valid_if).set_name("bad_cub_valid_if");
