...
using integral_to_realType =
  std::conditional_t<cuda::std::is_floating_point_v<T>, T, std::conditional_t<sizeof(T) * 8 <= 23, float, double>>;
...
public:
  ...
  __host__ __device__ result_type operator()(UniformRandomNumberGenerator& urng)
  {
    ...
    realType const result = _lower_bound < _upper_bound ? std::abs(super_t::operator()(urng)) + _lower_bound
...
struct value_generator
{
  ...
  {
    engine.discard(n);
    if constexpr (cuda::std::is_integral_v<T> && cuda::std::is_floating_point_v<decltype(dist(engine))>)
    {
      return cuda::std::clamp(static_cast<T>(cuda::std::round(dist(engine))), lower_bound, upper_bound);
    }
    else
    {
...
template <typename T, std::enable_if_t<cuda::std::is_integral_v<T> or cuda::std::is_floating_point_v<T>, T>* = nullptr>
  ...
  switch (dist_id)
  {
    case distribution_id::NORMAL:
      return [lower_bound, upper_bound, dist = make_normal_dist(lower_bound, upper_bound)](
        ...
        thrust::tabulate(
          thrust::device, result.begin(), result.end(), value_generator{lower_bound, upper_bound, engine, dist});
        return result;
      };
    ...
    default:
      CUDF_FAIL("Unsupported probability distribution");
  }
}
